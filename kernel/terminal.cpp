...
WithError<PageMapEntry*> SetNewPageMapIfNotPresent(PageMapEntry& entry) {
  if(entry.bits.present) { /* check entry is already set */
    return { entry.Pointer(), MAKE_ERROR(Error::kSuccess) };
  }

  /* As blow, entry is stil invalid */
...
WithError<size_t> SetupPageMap(
  ...
  while(num_4kpages > 0) {
    ...
    /* cary address to the next digit */
...
Error SetupPageMaps(LinearAddress4Level addr, size_t num_4kpages) { /* addr means Load Segment header address */
  auto pml4_table = reinterpret_cast<PageMapEntry*>(GetCR3()); /* CR3 register stores PML4 (highest) physical address */
  return SetupPageMap(pml4_table, 4, addr, num_4kpages).error;
}

Error CopyLoadSegments(Elf64_Ehdr* ehdr) {
  auto phdr = GetProgramHeader(ehdr);
  for(int i = 0; i < ehdr->e_phnum; i++) { /* e_phnum is the number of program headers */
    if(phdr[i].p_type != PT_LOAD) continue;

    LinearAddress4Level dest_addr;
    dest_addr.value = phdr[i].p_vaddr;
    const auto num_4kpages = (phdr[i].p_memsz + 4095) / 4096; /* calculate page by 4KB */
    ...
    memset(dst + phdr[i].p_filesz, 0, phdr[i].p_memsz - phdr[i].p_filesz); /* process remain bytes such as .bss section (buffer for non-initialized global variables )*/
  }
  return MAKE_ERROR(Error::kSuccess);
}

Error LoadELF(Elf64_Ehdr* ehdr) {
  /* check ELF file is executable */
  if(ehdr->e_type != ET_EXEC) {
    return MAKE_ERROR(Error::kInvalidFormat);
  }

  const auto addr_first = GetFirstLoadAddress(ehdr);
  /* check load segment address locates in the latter canonical address */
...
Error CleanPageMap(PageMapEntry* page_map, int page_map_level) {
  for(int i = 0; i < 512; i++) {
    ...
    if(auto err = memory_manager->Free(map_frame, 1)) return err;
...
Error Terminal::ExecuteFile(const fat::DirectoryEntry& file_entry, char* command, char* first_arg) {
  ...
  int ret = CallApp(argc.value, argv, 3 << 3 | 3, entry_addr,
      stack_frame_addr.value + 4096 - 8, &task.OSStackPointer()); /* stack alignment constraint */
...
void TaskTerminal(uint64_t task_id, int64_t data) {
  ...
  auto add_blink_timer = [task_id](unsigned long t) {
    time_manager->AddTimer(Timer{t + static_cast<int>(kTimerFreq * 0.5),
        1, task_id});
  };
  add_blink_timer(time_manager->CurrentTick());
